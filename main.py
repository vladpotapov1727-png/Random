...
def channel_keyboard(user_id):
    channels = user_channels.get(user_id, [])
    buttons = []
    for ch in channels:
        buttons.append([KeyboardButton(text=f"❌ {ch}")])
    buttons.append([KeyboardButton(text="➕ Добавить канал")])
...
async def create_raffle_button(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Можете добавить картинку, видео или GIF.\n"
        "📌 Только <b>1 медиафайл</b>",
        parse_mode="HTML"
    )
...
async def my_channels_button(message: Message):
    ...
    if not channels:
        ...
        await message.answer(text, reply_markup=channel_keyboard(user_id))
        return
    text = "📢 <b>Твои каналы</b>\n\n"
    for ch in channels:
        text += f"✅ {ch}\n"
    text += "\n❌ — удалить канал\n➕ — добавить новый"
    await message.answer(text, parse_mode="HTML", reply_markup=channel_keyboard(user_id))

@dp.message(F.text == "🆘 Служба поддержки")
async def support_button(message: Message):
    await message.answer(
        f"🆘 <b>Служба поддержки</b>\n\n"
        f"По всем вопросам пишите: {SUPPORT_TAG}",
        parse_mode="HTML",
        reply_markup=main_keyboard()
    )
...
async def delete_channel(message: Message):
    ...
    if channel in user_channels[user_id]:
        ...
        await message.answer(f"✅ Канал {channel} удалён!", reply_markup=channel_keyboard(user_id))
    else:
...
async def add_channel_start(message: Message, state: FSMContext):
    await message.answer(
        f"📢 Отправь название канала в формате <code>@channelname</code>\n\n"
        f"⚠️ Бот должен быть администратором в канале!",
        parse_mode="HTML",
...
@dp.message(ChannelStates.waiting_add)
async def add_channel_confirm(message: Message, state: FSMContext):
    if message.text == "🔙 Назад":
        await state.clear()
        await my_channels_button(message)
        return
    user_id = message.from_user.id
    ...
    save_channels(user_id, user_channels[user_id])
    await state.clear()
    await message.answer(f"✅ Канал {channel} добавлен!", reply_markup=channel_keyboard(user_id))

@dp.message(F.text == "🔙 Назад")
async def back_to_channels(message: Message):
    await my_channels_button(message)
...
async def ask_channels(message: Message, state: FSMContext):
    ...
    if not channels:
        await message.answer(
            "📢 <b>У тебя нет добавленных каналов!</b>\n\n"
            "Нажми «Пропустить» — тогда подписка не требуется.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="⏭ Пропустить каналы", callback_data="skip_channels")]
            ])
        )
        return
    text = "📢 <b>Добавьте каналы для подписки</b>\n\n"
    for ch in channels:
        text += f"✅ {ch}\n"
    text += "\nНажми «✅ Достаточно каналов», чтобы продолжить."
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Достаточно каналов", callback_data="channels_done")],
        [InlineKeyboardButton(text="➕ Добавить канал", callback_data="add_channel_from_raffle")]
    ])
    await message.answer(text, parse_mode="HTML", reply_markup=keyboard)
    await state.set_state(CreateRaffle.waiting_channels)

@dp.callback_query(F.data == "skip_channels")
...
@dp.callback_query(F.data == "add_channel_from_raffle")
async def add_channel_from_raffle(callback: CallbackQuery):
    await callback.message.answer(
        f"📢 Отправь название канала в формате <code>@channelname</code>\n\n"
        f"После добавления вернись к созданию конкурса.",
        parse_mode="HTML"
    )
    await callback.answer()

@dp.message(CreateRaffle.waiting_channels)
async def add_channel_from_raffle_message(message: Message, state: FSMContext):
    user_id = message.from_user.id
    channel = message.text.strip()
    if not channel.startswith("@"):
        await message.answer("❌ Канал должен начинаться с @")
    ...
    user_channels[user_id].append(channel)
    save_channels(user_id, user_channels[user_id])
    await message.answer(f"✅ Канал {channel} добавлен! Продолжи создание конкурса.")
    await ask_channels(message, state)
...
async def get_publish_channel(message: Message, state: FSMContext):
    ...
    try:
        member = await bot.get_chat_member(channel, bot.id)
        if member.status not in ["administrator", "creator"]:
            await message.answer(f"❌ Бот не админ в {channel}! Добавьте бота в канал.")
            return
    except:
        await message.answer(f"❌ Канал {channel} не найден! Проверь название.")
        return
...
async def admin_all(callback: CallbackQuery):
    ...
    for rid, r in raffles.items():
        ...
        if r.get("selected_winners"):
            text += f"   👑 Выбрано: {len(r['selected_winners'])} победителей\n"
        if r.get("winner"):
            text += f"   🏆 Победитель: @{r['winner']['username']}\n"
        text += "\n"
...
async def admin_pick(callback: CallbackQuery, state: FSMContext):
    ...
    for rid in active:
        r = raffles[rid]
        selected = " 👑" if r.get("selected_winners") else ""
        text += f"#{rid}: {r.get('publish_channel', 'неизвестно')} — {len(r.get('participants', []))} участников{selected}\n"
...
async def admin_pick_raffle(message: Message, state: FSMContext):
    ...
    # Показываем список участников с номерами
    participants = raffle.get("participants", [])
    text = f"🎲 <b>Выбери победителей</b>\n\n"
    text += f"📢 {raffle.get('publish_channel', 'неизвестно')}\n"
    text += f"👥 Участников: {len(participants)}\n\n"
    text += "<b>Список участников:</b>\n"
    for i, p in enumerate(participants, 1):
        text += f"{i}. @{p['username']} ({p['first_name']})\n"
    
    if raffle.get("selected_winners"):
        text += f"\n✅ <b>Уже выбрано:</b> {len(raffle['selected_winners'])} победителей\n"
        text += "Чтобы изменить выбор — выбери новых (они заменят старых).\n"
    else:
        text += "\nНапиши номера через запятую.\n"
        text += "Пример: <code>3, 7, 12</code>"
    
    await message.answer(text, parse_mode="HTML")
    await state.set_state(AdminPickStates.waiting_winners)

@dp.message(AdminPickStates.waiting_winners)
async def admin_pick_winners(message: Message, state: FSMContext):
    ...
    try:
        ...
        winners = []
        for num in nums:
            if num < 1 or num > len(participants):
                await message.answer(f"❌ Номер {num} не существует! Всего: {len(participants)}")
                return
            winners.append(participants[num - 1])
    except:
        await message.answer("❌ Неправильный формат! Пример: <code>3, 7, 12</code>", parse_mode="HTML")
        return
    
    # Сохраняем выбранных победителей
    ...
    if len(winners) == 1:
        ...
        text += f"📌 Он будет объявлен, когда конкурс завершится.\n\n"
        text += "🔄 Чтобы изменить — снова нажми «Выбрать победителей»."
    else:
        text = f"✅ <b>Ты выбрал {len(winners)} победителей:</b>\n\n"
...
async def broadcast_send(message: Message, state: FSMContext):
    ...
    for user_id in users:
        try:
            await bot.send_message(user_id, message.html_text + f"\n\n📢 {SUPPORT_TAG}", parse_mode="HTML")
            sent += 1
        except:
            failed += 1
...
async def announce_winner(raffle_id: int, from_join: bool = False):
    ...
    try:
        await bot.send_message(chat_id=raffle["chat_id"], text=text + f"\n\n📢 {SUPPORT_TAG}", parse_mode="HTML")
    except:
        pass
    
    try:
        await bot.send_message(ADMIN_ID, text=f"✅ <b>Победители объявлены!</b>\n\n#{raffle_id}: {raffle.get('publish_channel')}", parse_mode="HTML")
    except:
        pass
...
# ===== ЗАПУСК =====
async def main():
    print(f"🤖 Бот giveawayrnd_bot запущен!")
